  ...
  return (
    ...
    >
      <div className="bg-surface border border-border rounded-3xl p-8 max-w-sm w-full text-center">
        ...
        <div className="font-heading text-5xl font-black text-yellow-400 mb-0.5">
          {info.label}
        </div>
        ...
        <div className="text-left text-lg space-y-1.5 mb-5">
          {features.map((f) => (
        ...
        {error && (
          <div className="text-red-400 text-lg mb-3 p-2 bg-red-500/10 rounded-lg">
            {error}
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-3 mb-3">
          ...
          />
          <span className="text-muted text-lg">СБП</span>
          <span className="text-muted text-lg">Мир</span>
        </div>

        <div className="text-muted text-lg mb-3">
          Безопасная оплата через Prodamus · доступ сразу после оплаты
        </div>

        <button
          onClick={onClose}
          className="w-full py-2 text-muted text-lg hover:text-white transition-colors"
        >
          Отмена
        </button>
      </div>
