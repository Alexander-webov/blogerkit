  ...
  return (
    <div className="min-h-screen bg-bg">
      <nav className="sticky top-0 z-40 h-14 flex items-center justify-between px-6 bg-bg/95 backdrop-blur border-b border-border">
        ...
        <div className="text-muted text-lg">🤖 AI-генератор заголовков</div>
      </nav>
      ...
      <div className="relative overflow-hidden pt-10 pb-6 px-6 text-center">
        ...
        <div className="relative max-w-xl mx-auto">
          ...
          <p className="text-muted text-sm">
            8 вариантов · с оценкой CTR · бесплатно
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-6 pb-16">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {EXAMPLES.map((e) => (
              <button
                key={e}
                onClick={() => setTopic(e)}
                className="px-2.5 py-1 text-lg border border-border rounded-full text-muted hover:text-white hover:border-white/20 transition-colors bg-transparent cursor-pointer"
              >
                {e}
              </button>
            ))}
          ...
          <p className="text-center text-muted text-lg mt-2">
          ...
          <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl mb-4">
            ...
            {error.includes("ANTHROPIC_API_KEY") && (
              <div className="mt-2 text-lg text-muted">
                Открой{" "}
                <code className="bg-white/10 px-1 rounded">.env.local</code> и
                добавь:
                <br />
                <code className="bg-white/10 px-1 rounded mt-1 block">
          ...
          <div className="animate-fadeUp">
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <div
                        className={`font-heading font-black text-sm ${scoreColor(r.score)}`}
                      >
                        {r.score}%
                      </div>
                      <button
                        onClick={() => copy(i, r.title)}
                        className="px-2.5 py-1 bg-white/5 border border-border rounded-lg text-lg hover:bg-white/10 transition-colors cursor-pointer"
                      >
                        {copied === i ? "✓" : "Копировать"}
                      </button>
                    </div>
                  </div>
                  <div className="text-muted text-lg">💡 {r.why}</div>
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="text-muted text-lg">CTR</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 p-5 bg-card border border-border rounded-2xl text-center">
              <div className="font-heading text-sm font-bold mb-1">
                Хочешь проверить конкурентов?
              </div>
              <p className="text-muted text-lg mb-3">
                Посмотри какие заголовки набирают просмотры в твоей нише
              </p>
              <div className="flex gap-2 justify-center">
                <Link
                  href="/analyze"
                  className="px-4 py-2 bg-accent text-white font-bold text-lg rounded-xl hover:opacity-85 transition-opacity"
                >
                  Анализ конкурентов →
                </Link>
                <Link
