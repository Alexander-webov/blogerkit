...
interface ChannelData {
  channel: { id: string; name: string; description: string; avatar: string; subs: number; views: number; videoCount: number; createdAt: string; country: string }
  stats:   { avgViews: number; engRate: number; growthRate: number }
  videos:  any[]; topVideo: any; worstVideo: any
}
...
export default function ChannelAnalysisTool() {
  ...
  function generateAdvice(d: ChannelData) {
    ...
    if (stats.engRate < 2)   tips.push({icon:'⚠️', text:'Вовлечённость ниже 2% — добавляй призывы к лайку и комментарию в конце каждого видео.'})
    if (stats.engRate >= 5)  tips.push({icon:'🔥', text:'Отличная вовлечённость! Можно просить за рекламу на 20–30% выше средней цены по нише.'})
    if (stats.growthRate < 5) tips.push({icon:'📉', text:'Просмотры сильно ниже числа подписчиков — обнови обложки старых видео и улучши заголовки.'})
    if (topVideo)             tips.push({icon:'💡', text:`Лучшее видео «${topVideo.title.slice(0,45)}…» набрало ${fmt(topVideo.views)} просм. Сними продолжение или похожее видео.`})
    if (worstVideo && topVideo && worstVideo.views < topVideo.views * 0.1)
      tips.push({icon:'🗑️', text:`Видео «${worstVideo.title.slice(0,40)}…» слабое (${fmt(worstVideo.views)} просм). Обнови обложку или удали.`})
    const avgLen = videos.reduce((s,v)=>s+(v.title?.length||0),0)/(videos.length||1)
    if (avgLen < 40) tips.push({icon:'📝', text:'Заголовки короткие — добавляй больше ключевых слов. Оптимум 55–70 символов.'})
    if (avgLen > 75) tips.push({icon:'✂️', text:'Заголовки слишком длинные — YouTube обрезает их. Укладывайся в 60–70 символов.'})
    return tips
  }
    ...
    <div className="min-h-screen bg-bg">
      ...
      {showPay && (
        <PaywallModal
          ...
          onPaid={() => { setShowPay(false); analyze() }}
        />
      )}
      ...
      <nav className="sticky top-0 z-40 h-14 flex items-center justify-between px-6 bg-bg/95 backdrop-blur border-b border-border">
        ...
        <div className="text-muted text-xs">📡 Анализ канала</div>
      </nav>
      ...
      <div className="max-w-3xl mx-auto px-6 pb-16">
        ...
        {!paid && !data && (
          <div className="bg-card border border-border rounded-2xl p-8 text-center">
            ...
            <h2 className="font-heading text-xl font-black mb-3">Что ты получишь</h2>
        ...
        {data && (
          <div className="animate-fadeUp space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl p-5 flex gap-4">
              {data.channel.avatar && (
                <div className="relative w-16 h-16 rounded-full overflow-hidden flex-shrink-0 bg-surface">
                  <Image src={data.channel.avatar} alt={data.channel.name} fill className="object-cover" sizes="64px" unoptimized />
                </div>
              )}
              <div>
                <div className="font-heading text-xl font-black mb-1">{data.channel.name}</div>
                <div className="flex flex-wrap gap-3 text-xs text-muted">
                  <span>📅 Создан {new Date(data.channel.createdAt).getFullYear()}</span>
                  {data.channel.country && <span>🌍 {data.channel.country}</span>}
                </div>
                {data.channel.description && (
                  <div className="text-muted text-xs mt-2 line-clamp-2">{data.channel.description}</div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                {label:'Подписчики',      val:fmt(data.channel.subs),         sub:'всего'},
                {label:'Просмотры',       val:fmt(data.channel.views),         sub:'за всё время'},
                {label:'Ср. просмотры',   val:fmt(data.stats.avgViews),        sub:'за видео'},
                {label:'Видео',           val:String(data.channel.videoCount), sub:'опубликовано'},
              ].map(s => (
                <div key={s.label} className="bg-card border border-border rounded-xl p-4 text-center">
                  <div className="font-heading text-xl font-black text-accent mb-0.5">{s.val}</div>
              ...
              {(() => {
                ...
                return (<>
                  <div className="bg-card border border-border rounded-xl p-5">
                    <div className="text-muted text-xs uppercase tracking-widest mb-2">Вовлечённость</div>
                    <div className={`font-heading text-3xl font-black mb-1 ${eng.color}`}>{data.stats.engRate}%</div>
                    <div className={`text-sm font-bold ${eng.color} mb-2`}>{eng.label}</div>
                    <div className="h-1.5 bg-border rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{width:`${eng.pct}%`, background: eng.color.includes('green')?'#10b981':eng.color.includes('yellow')?'#fbbf24':'#ef4444'}} />
                    </div>
                  </div>
                  <div className="bg-card border border-border rounded-xl p-5">
                    <div className="text-muted text-xs uppercase tracking-widest mb-2">Охват аудитории</div>
                    <div className={`font-heading text-3xl font-black mb-1 ${gr.color}`}>{data.stats.growthRate}%</div>
                    <div className={`text-sm font-bold ${gr.color}`}>{gr.label}</div>
                    <div className="text-muted text-xs mt-1">просмотры / подписчики</div>
                  </div>
                </>)
              })()}
            </div>
            ...
            {data.topVideo && (
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  {label:'🏆 Лучшее видео', v:data.topVideo,   border:'border-green-400/20 bg-green-400/5'},
                  {label:'📉 Слабое видео',  v:data.worstVideo, border:'border-red-400/20 bg-red-400/5'},
                ].map(({label,v,border}) => v && (
                  <a key={label} href={v.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="flex gap-3">
                      {v.thumb && (
                        <div className="relative w-20 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-black">
                          <Image src={v.thumb} alt={v.title} fill className="object-cover" sizes="80px" unoptimized />
                        </div>
                      )}
                      <div>
                        <div className="font-semibold text-xs line-clamp-2 mb-1">{v.title}</div>
                        <div className="text-muted text-xs">{fmt(v.views)} просм · {daysAgo(v.publishedAt)}</div>
                      </div>
                    </div>
                  </a>
                ))}
            ...
            {/* AI ADVICE */}
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="font-heading text-sm font-bold mb-3">🤖 AI-советы</div>
              <div className="space-y-2">
                {generateAdvice(data).map((tip,i) => (
                  <div key={i} className="flex gap-2 text-xs p-2.5 bg-white/3 rounded-lg border-l-2 border-accent/40">
                    <span>{tip.icon}</span><span className="text-muted">{tip.text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* VIDEOS LIST */}
            <div>
              <div className="text-muted text-xs uppercase tracking-widest mb-3">Последние {data.videos.length} видео</div>
              <div className="flex flex-col gap-2">
                {data.videos.slice(0,10).map((v,i) => (
                  <a key={v.id} href={v.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="text-muted font-mono text-xs w-5 text-center">{i+1}</div>
                    {v.thumb && (
                      <div className="relative w-20 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-black">
                        <Image src={v.thumb} alt={v.title} fill className="object-cover" sizes="80px" unoptimized />
                      </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-xs line-clamp-1 group-hover:text-white/80 mb-0.5">{v.title}</div>
                      <div className="text-muted text-xs">{fmt(v.views)} просм · {daysAgo(v.publishedAt)}</div>
                    </div>
                  </a>
                ))}
