...
const FAQS = [
  {
    q: "Как называть цену рекламодателю?",
    a: "Называй верхнюю границу диапазона. Рекламодатель начнёт торговаться — у тебя будет пространство.",
  },
  {
    q: "Когда можно начинать брать рекламу?",
    a: "Практически — рекламодатели начинают интересоваться от 1 000–5 000 подписчиков если хорошая вовлечённость.",
  },
  {
    q: "Где искать рекламодателей?",
    a: "ВКонтакте: Маркет-платформа. Telegram: Telega.in, TGStat. YouTube: Напрямую писать брендам. Универсально: GetBlogger, Epicstars.",
  },
  {
    q: "Что включить в медиакит?",
    a: "Количество подписчиков, охват поста/видео, вовлечённость, демография аудитории, примеры прошлых интеграций и цены.",
  },
];
...
export default function CalculatorTool() {
  ...
  function calculate() {
    ...
    if (s > 0 && v / s < 0.05)
      tips.push({
        icon: "⚠️",
        text: "Вовлечённость ниже среднего — учти это при переговорах.",
      });
    if (s > 0 && v / s >= 0.15)
      tips.push({
        icon: "🔥",
        text: "Отличная вовлечённость! Можешь просить на 20–30% больше.",
      });
    if (s >= 100000)
      tips.push({
        icon: "💎",
        text: "Канал 100k+ — создай медиакит, рекламодатели сами будут писать.",
      });
    if (s < 5000)
      tips.push({
        icon: "💡",
        text: "Первым рекламодателям сделай скидку 20–30% в обмен на отзыв.",
      });
    tips.push({
      icon: "📋",
      text: "Называй верхнюю границу при переговорах — рекламодатель всё равно поторгуется.",
    });
  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <nav className="sticky top-0 z-40 h-14 flex items-center justify-between px-6 bg-bg/95 backdrop-blur border-b border-border">
        ...
        <div className="text-muted text-lg">💰 Калькулятор рекламы</div>
      </nav>
      ...
      <div className="relative overflow-hidden pt-12 pb-8 px-6 text-center">
        ...
        <div className="relative max-w-xl mx-auto">
          ...
          <p className="text-muted text-sm">
            Введи данные канала — получи рекомендуемую цену для рекламодателей
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-16 grid md:grid-cols-[1fr_300px] gap-5 items-start">
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          <div className="font-heading text-sm font-bold mb-5">
            💰 Данные канала
          </div>

          {/* PLATFORM */}
          <div className="mb-4">
            <label className="text-muted text-lg uppercase tracking-widest block mb-2">
              Платформа
            </label>
            <div className="grid grid-cols-5 gap-2">
              {PLATFORMS.map((p) => (
                <button
                  ...
                  className={`py-2 rounded-xl border text-lg font-bold transition-all cursor-pointer ${platform === p.id ? "bg-accent/10 border-accent/40 text-accent" : "border-border text-muted hover:text-white bg-transparent"}`}
                >
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="text-muted text-lg uppercase tracking-widest block mb-1.5">
                Подписчики
              </label>
              <input
                type="number"
          ...
          {result && (
            <div
              id="result"
              className="mt-4 p-5 bg-gradient-to-br from-accent2/8 to-accent/6 border border-accent2/25 rounded-xl animate-fadeUp"
            >
              <div className="text-muted text-lg uppercase tracking-widest mb-1">
                Рекомендуемая цена рекламы
              </div>
              ...
              <div className="text-muted text-lg mb-4">
                {result.typeName} · {result.platform} · {result.nicheName}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {[
                  {
                    label: "Минимум",
                    val: fmtPrice(result.min),
                    color: "text-green-400",
                  },
                  {
                    label: "Максимум",
                    val: fmtPrice(result.max),
                    color: "text-yellow-400",
                  },
                  {
                    label: "CPM",
                    val: fmtPrice(result.cpm),
                    color: "text-white",
                  },
                  {
                    label: "Коэфф. ниши",
                  ...
                  <div key={item.label} className="bg-white/4 rounded-xl p-3">
                    <div className="text-muted text-lg mb-1">{item.label}</div>
              ...
              <div className="flex flex-col gap-2">
                {result.tips.map((tip, i) => (
                  <div
                    key={i}
                    className="flex gap-2 text-lg text-muted p-2 bg-white/3 rounded-lg border-l-2 border-accent2"
                  >
                    <span>{tip.icon}</span>
                    <span>{tip.text}</span>
                  </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <table className="w-full">
              <thead>
                <tr>
                  <th className="text-left text-muted text-lg pb-2">
                    Платформа
                  </th>
                  <th className="text-right text-muted text-lg pb-2">CPM</th>
                </tr>
              </thead>
              <tbody>
                {CPM_TABLE.map((row) => (
                  <tr key={row.name} className="border-t border-white/4">
                    <td className="py-2 text-lg">{row.name}</td>
                    <td className="py-2 text-right font-heading text-lg font-bold text-yellow-400">
                      {row.range}
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            {NICHE_MULTS.map((n) => (
              ...
              >
                <span className="text-muted text-lg">{n.name}</span>
                <span className={`font-bold text-lg ${n.color}`}>{n.mult}</span>
              </div>
            ))}
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            {FAQS.map((faq, i) => (
              <div key={i} className="border-b border-white/4 last:border-0">
                ...
                >
                  <span className="text-lg font-semibold pr-2">{faq.q}</span>
                  <span
                    className={`text-muted text-lg transition-transform ${openFaq === i ? "rotate-180" : ""}`}
                  >
                    ▼
                  </span>
                </button>
                {openFaq === i && (
                  <p className="text-muted text-lg pb-3 leading-relaxed">
                    {faq.a}
                  </p>
                )}
              </div>
