...
const NICHE_QUERIES: Record<string, string[]> = {
  auto:       ['авто обзор 2025', 'тест драйв', 'автомобиль купил'],
  finance:    ['инвестиции 2025', 'как заработать', 'крипта'],
  gaming:     ['игры 2025', 'обзор игры', 'прохождение'],
  ...
  tech:       ['обзор гаджет', 'технологии 2025', 'смартфон'],
...
export async function GET(req: NextRequest) {
  ...
  if (!YT_KEY) return NextResponse.json({ error: 'No API key' }, { status: 500 })
  ...
  try {
    ...
    if (!searchData.items?.length) return NextResponse.json({ videos: [], query })
