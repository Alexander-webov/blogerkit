...
function analyzePatterns(videos: Video[]) {
  ...
  const hasEmoji = titles.filter((t) =>
    [...t].some((ch) => ch.codePointAt(0)! > 127),
  ).length;
    ...
    <div className="min-h-screen bg-bg">
      ...
      <nav className="sticky top-0 z-40 h-14 flex items-center justify-between px-6 bg-bg/95 backdrop-blur border-b border-border">
        ...
        >
          <div
            ...
            style={{ background: activeNiche.color }}
          />
          БлогерКит
        </Link>
        <div className="flex items-center gap-2 text-muted text-lg">
          <span className="animate-pulse2">●</span> Данные за последние 7 дней ·
          YouTube
        </div>
      </nav>
      ...
      <div className="relative overflow-hidden pt-12 pb-8 px-6 text-center">
        <div
          ...
          style={{
            background: `radial-gradient(ellipse 50% 30% at 50% 0%, ${activeNiche.color}11 0%, transparent 70%)`,
          }}
        />
        <div className="relative max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 text-lg text-muted mb-4">
          ...
          <h1 className="font-heading text-3xl md:text-4xl font-black tracking-tight mb-3">
            ...
            <span
              ...
              style={{
                backgroundImage: `linear-gradient(135deg, ${activeNiche.color}, ${activeNiche.color}88)`,
              }}
            >
              YouTube
            </span>
          ...
          <p className="text-muted text-sm">
            Что набирает просмотры прямо сейчас · Реальные данные
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-16">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {NICHES.map((n) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-lg font-bold transition-all cursor-pointer ${niche === n.id ? "text-black" : "border-border text-muted hover:text-white bg-transparent"}`}
              style={
                niche === n.id
        ...
        {status === "loading" && (
          <div className="text-center py-20">
            <div className="relative w-12 h-12 mx-auto mb-4">
              <div
                className="w-12 h-12 rounded-full border-[3px] border-border animate-spin"
                style={{ borderTopColor: activeNiche.color }}
              />
            </div>
            <p className="text-muted text-sm">
              Загружаем тренды {activeNiche.icon} {activeNiche.label}...
            </p>
          </div>
        )}
        ...
        {status === "error" && (
          <div className="max-w-md mx-auto py-10 text-center">
            ...
            <div className="text-muted text-lg mb-4">{error}</div>
            <button
              onClick={() => load(niche)}
              className="px-4 py-2 border border-border rounded-lg text-sm hover:border-white/20 transition-colors"
            >
              Попробовать снова
            </button>
          </div>
          ...
          <div className="animate-fadeUp">
              ...
              {[
                {
                  ...
                  val: fmt(patterns.avgViews),
                  sub: "за 7 дней",
                },
                {
                  label: "Средняя длина загол.",
                  val: `${patterns.avgLen} симв`,
                  sub: "оптимум 50–70",
                },
                {
                  label: "С числами в заголовке",
                  val: `${Math.round((patterns.hasNumbers / patterns.total) * 100)}%`,
                  sub: "конвертируют лучше",
                },
                {
                  label: "С CAPS в заголовке",
                  val: `${Math.round((patterns.hasCaps / patterns.total) * 100)}%`,
                  sub: "для шок-контента",
                },
              ].map((s) => (
                <div
                  key={s.label}
                ...
                >
                  <div
                    className="font-heading text-xl font-black mb-0.5"
                    style={{ color: activeNiche.color }}
                  >
                    {s.val}
                  </div>
            ...
            {patterns.topVideo && (
              ...
              >
                <div className="relative w-40 h-24 rounded-xl overflow-hidden flex-shrink-0 bg-black">
                  <Image
                    src={patterns.topVideo.thumb}
                    alt={patterns.topVideo.title}
                    fill
                    className="object-cover"
                    sizes="160px"
                  />
                  <div
                    className="absolute top-2 left-2 px-2 py-0.5 rounded text-lg font-black text-black"
                    style={{ background: activeNiche.color }}
                  >
                    🔥 ТРЕНД
                  </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-lg text-muted mb-1">
                    Самый быстрорастущий за 7 дней
                  </div>
                  ...
                  <div className="flex gap-4 text-lg">
                    <span
                      ...
                      style={{ color: activeNiche.color }}
                    >
                      {fmt(patterns.topVideo.views)} просмотров
                    </span>
                    <span className="text-muted">
                      {patterns.topVideo.channel}
                    </span>
                    <span className="text-muted">
                      {daysAgo(patterns.topVideo.publishedAt)}
                    </span>
                  </div>
                </div>
              </a>
            ...
            {patterns.topWords.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-4 mb-6">
                <div className="text-lg text-muted uppercase tracking-widest mb-3">
                  💬 Частые слова в заголовках трендов
                </div>
                <div className="flex flex-wrap gap-2">
                  {patterns.topWords.map(([word, count]) => (
                    <div
                      key={word}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-surface text-lg"
                    >
                ...
                <p className="text-muted text-lg mt-3">
                  💡 Попробуй добавить эти слова в свои заголовки — они работают
                  прямо сейчас
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="text-lg text-muted">
                Запрос: <span className="text-white">«{query}»</span> ·{" "}
                {videos.length} видео
              </div>
              <div className="flex gap-2">
                {[
                  ["trend", "🔥 Тренд"],
                  ["views", "👁 Просмотры"],
                  ["engagement", "❤️ Вовлечённость"],
                ].map(([v, l]) => (
                  <button
                    key={v}
                    onClick={() => setSort(v as any)}
                    className={`px-3 py-1.5 text-lg rounded-lg border transition-colors cursor-pointer ${sort === v ? "border-white/20 text-white bg-white/5" : "border-border text-muted hover:text-white bg-transparent"}`}
                  >
                    {l}
                  </button>
                ))}
              ...
              {sorted.map((v, i) => {
                ...
                return (
                  ...
                  >
                    {/* RANK */}
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-lg font-black flex-shrink-0 self-center ${i === 0 ? "text-black" : i === 1 ? "text-black" : i === 2 ? "text-black" : "text-muted bg-white/5"}`}
                      style={
                        i < 3
                          ? {
                              background:
                    ...
                    {/* THUMB */}
                    <div
                      className="relative w-32 h-18 rounded-lg overflow-hidden flex-shrink-0 bg-black"
                    ...
                    >
                      <Image
                        ...
                        sizes="128px"
                      />
                      <div className="absolute bottom-1 right-1 bg-black/80 text-white text-lg px-1 rounded">
                        {v.duration}
                      </div>
                    </div>
                    ...
                    {/* INFO */}
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm leading-snug mb-1 line-clamp-2 group-hover:text-white/80 transition-colors">
                        {v.title}
                      </div>
                      <div className="text-muted text-lg mb-2">
                        {v.channel} · {daysAgo(v.publishedAt)}
                      </div>
                      ...
                      {v.tags.length > 0 && (
                        <div className="flex gap-1 mt-1.5 flex-wrap">
                          {v.tags.slice(0, 3).map((tag) => (
                            <span
                              key={tag}
                              className="px-1.5 py-0.5 bg-white/4 border border-white/8 rounded text-lg text-muted"
                            >
                              {tag}
                            </span>
                          ))}
            ...
            <div className="mt-8 p-5 bg-card border border-border rounded-2xl text-center">
              <div className="font-heading text-sm font-bold mb-2">
                Хочешь снять видео по этому тренду?
              </div>
              <p className="text-muted text-lg mb-4">
                Проанализируй конкурентов подробнее или создай обложку прямо
                сейчас
              </p>
              <div className="flex gap-3 justify-center flex-wrap">
                <Link
                  href="/analyze"
                  className="px-4 py-2 bg-accent text-white text-lg font-bold rounded-xl hover:opacity-85 transition-opacity"
                >
                  🔍 Анализировать конкурентов
                </Link>
                <Link
                  href="/covers"
                  className="px-4 py-2 border border-border text-lg rounded-xl hover:border-white/20 transition-colors"
                >
                  🎨 Создать обложку
                </Link>
              </div>
