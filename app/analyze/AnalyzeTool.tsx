...
function Paywall({
  ...
  return (
    ...
    >
      <div
        className="bg-surface border border-border rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center"
      ...
      >
        ...
        {state === "offer" && (
          <div className="animate-fadeUp">
            ...
            <p className="text-muted text-lg leading-relaxed mb-4">
              Нашли реальные видео конкурентов с YouTube.
              <br />
              Получи полный отчёт за один платёж.
            </p>
            <div className="inline-block px-3 py-1 rounded-lg border border-accent/25 bg-accent/10 text-accent text-lg font-semibold mb-4">
              🔎 {query}
            </div>
            <div className="font-heading text-5xl font-black text-yellow-400 leading-none mb-1">
              29 ₽
            </div>
            <div className="text-muted text-lg mb-5">
              разовый платёж · результат сразу
            </div>

            <div className="flex flex-col gap-2 mb-5 text-left bg-white/3 border border-border rounded-xl p-4">
              {[
                "Топ-10 реальных видео конкурентов",
                "Просмотры, лайки, комментарии",
                "Инсайты и паттерны заголовков",
                "Анализ цветов обложек",
                "Лучшее время публикации",
              ].map((f) => (
                <div key={f} className="flex items-center gap-2 text-lg">
            ...
            <button
              onClick={onClose}
              className="w-full py-2 border border-border text-muted text-lg rounded-xl hover:text-white transition-colors"
            >
              Отмена
            </button>
          </div>
        ...
        {state === "card" && (
          <div className="animate-fadeUp text-left">
            <button
              ...
              className="text-muted text-lg mb-4 flex items-center gap-1 hover:text-white transition-colors bg-transparent border-none cursor-pointer"
            >
              ← Назад
            </button>
            <div className="font-heading text-sm font-bold text-center mb-5">
              💳 Оплата
            </div>
            <div className="mb-3">
              <label className="text-muted text-lg uppercase tracking-widest block mb-1">
                Номер карты
              </label>
              <input
                className="w-full px-3 py-2.5 bg-bg border border-border rounded-lg text-sm outline-none focus:border-accent2 transition-colors"
                placeholder="0000 0000 0000 0000"
                maxLength={19}
                value={card.num}
                onChange={(e) =>
                  setCard({ ...card, num: formatCardNum(e.target.value) })
                }
              />
            </div>
            ...
            <div className="text-center text-muted text-lg mb-4 flex items-center justify-center gap-1">
        ...
        {state === "processing" && (
          <div className="animate-fadeUp py-4">
            ...
            <div className="text-muted text-lg mb-6">Не закрывай страницу</div>
            <div className="flex flex-col gap-2 text-left">
              {steps.map((s, i) => (
                <div
                  key={s}
                  className={`flex items-center gap-2 text-lg transition-colors ${step > i ? "text-green-400" : "text-muted"}`}
                >
        ...
        <div className="relative overflow-hidden pt-12 pb-8 px-6 text-center">
          ...
          <div className="relative max-w-xl mx-auto">
            ...
            <p className="text-muted text-sm mb-6">
              Реальные данные с YouTube · 29 ₽ за запрос
            </p>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {HINTS.map((h) => (
                <button
                  ...
                  className="px-3 py-1 text-lg border border-border rounded-full text-muted hover:text-white hover:border-white/20 transition-colors bg-transparent cursor-pointer"
                >
                  {h}
                </button>
              ))}
          ...
          {status === "loading" && (
            <div className="text-center py-20">
              ...
              <p className="text-muted text-sm">
                Загружаем реальные данные с YouTube...
              </p>
            </div>
          )}
          ...
          {status === "idle" && !error && (
            <div className="text-center py-20">
              ...
              <div className="text-muted text-sm">
                Например "авто обзор" или "фитнес для начинающих"
              </div>
            </div>
          )}
          ...
          {status === "results" && videos.length > 0 && (
            <div className="animate-fadeUp">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                {[
                  ...
                  >
                    ...
                    <div className="text-muted text-lg mt-1">{s.label}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-2 mb-4">
                {[
                  ["views", "По просмотрам"],
                  ["engagement", "По вовлечённости"],
                ].map(([v, l]) => (
                  <button
                    key={v}
                    onClick={() => setSort(v)}
                    className={`px-3 py-1.5 text-lg rounded-lg border transition-colors cursor-pointer ${sort === v ? "bg-accent/10 border-accent/30 text-accent" : "border-border text-muted hover:text-white bg-transparent"}`}
                  >
                    {l}
                  </button>
                ))}
                ...
                {sorted.map((v, i) => (
                  ...
                  >
                    <div className="relative flex-shrink-0 w-36 h-20 rounded-lg overflow-hidden bg-black">
                      ...
                      <div
                        className={`absolute top-1 left-1 w-6 h-6 rounded-full flex items-center justify-center text-lg font-black ${i === 0 ? "bg-yellow-400 text-black" : i === 1 ? "bg-gray-400 text-black" : i === 2 ? "bg-orange-500 text-black" : "bg-black/60 text-white"}`}
                      >
                        {i + 1}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm leading-snug mb-1 line-clamp-2 group-hover:text-accent3 transition-colors">
                        {v.title}
                      </div>
                      ...
                      <div className="flex gap-4 text-lg">
                        <span className="text-accent font-bold">
                          {fmt(v.views)} просмотров
                        </span>
                        <span className="text-muted">
                          {fmt(v.likes)} лайков
                        </span>
                        <span className="text-muted">
                          {fmt(v.comments)} комментов
                        </span>
                      </div>
                    </div>
                  </a>
