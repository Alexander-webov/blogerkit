...
export default function CropTool() {
  ...
  async function recordSegment(
    ...
    await new Promise<void>((resolve) => {
      ...
      recorder.onstop = () => {
        ...
        if (blob.size < 1000) {
          alert(
            "Файл пустой или слишком маленький. Убедись что используешь Chrome или Edge, и нажми Play перед экспортом.",
          );
        } else {
  ...
  async function exportCrop() {
    ...
    if (typeof MediaRecorder === "undefined") {
      alert(
        "Твой браузер не поддерживает запись видео. Используй Chrome или Edge.",
      );
      return;
    }
    setProcessing(true);
    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 pb-16">
        <div className="text-center mb-6">
          ...
          <p className="text-muted text-sm">Обрезать · Нарезка · Фильтры</p>
        </div>
              ...
              <div className="bg-card border border-border rounded-xl p-3 mb-3">
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${isFinite(duration) && duration > 0 ? (currentTime / duration) * 100 : 0}%`,
                    }}
                  />
                  {splitPoints.map((p, i) => (
                    <div
                ...
                <div className="bg-card border border-border rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-3 mb-4 flex-wrap">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-red-500/10 text-red-400 text-xs rounded-lg border border-red-500/20"
                    >
                      Сбросить
                    </button>
                  </div>
                  ...
                  <div className="p-3 bg-yellow-400/5 border border-yellow-400/20 rounded-lg text-xs text-yellow-400/80">
                    ⏱ Экспорт идёт в реальном времени — видео будет
            ...
            <div className="flex flex-col gap-4">
              ...
              {!paid && (
                ...
                >
                  ...
                  <div className="text-muted text-xs">Нарезка · Фильтры</div>
                </button>
              )}
            </div>
          </div>
