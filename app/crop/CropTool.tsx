...
export default function CropTool() {
  ...
  async function exportCrop() {
    ...
    const scaleX=vid.videoWidth/vidW, scaleY=vid.videoHeight/vidH
    ...
    if (!supported) {
      ...
      ctx.filter=filter.css||'none'
      ctx.drawImage(vid,cropX*scaleX,cropY*scaleY,cropW*scaleX,cropH*scaleY,0,0,ratio.w,ratio.h)
      const a=document.createElement('a')
      a.href=out.toDataURL('image/png'); a.download='frame.png'; a.click()
      return
    }
    ...
    let t=0
    const step=async()=>{
      if(t>=vid.duration){ recorder.stop(); return }
      vid.currentTime=t
      await new Promise(r=>{vid.onseeked=r})
      ctx.filter=filter.css||'none'
      ctx.drawImage(vid,cropX*scaleX,cropY*scaleY,cropW*scaleX,cropH*scaleY,0,0,ratio.w,ratio.h)
      ...
      if(sub){
        ...
        ctx.textAlign='center'; ctx.fillText(sub.text,ratio.w/2,ratio.h-42)
        ctx.textAlign='left'
      }
      t+=1/30; setProgress(Math.round(t/vid.duration*100))
      setTimeout(step,0)
    }
    await step()
  }
  ...
  async function exportSplit() {
    ...
    for(let i=0;i<segs.length-1;i++){
      const start=segs[i], end=segs[i+1]
      const out=document.createElement('canvas')
      out.width=ratio.w; out.height=ratio.h
      const ctx=out.getContext('2d')!
      ...
      await new Promise<void>(resolve=>{
        ...
        vid.onseeked=async()=>{
          ...
          let t=start
          const step=async()=>{
            if(t>=end){recorder.stop();return}
            vid.currentTime=t
            await new Promise(r=>{vid.onseeked=r})
            ctx.filter=filter.css||'none'
            ctx.drawImage(vid,cropX*scaleX,cropY*scaleY,cropW*scaleX,cropH*scaleY,0,0,ratio.w,ratio.h)
            t+=1/30
            setProgress(Math.round(((i+(t-start)/(end-start))/(segs.length-1))*100))
            setTimeout(step,0)
          }
          await step()
        }
    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 pb-16">
        <div className="text-center mb-6">
          ...
          <p className="text-muted text-sm">Кроп · Нарезка · Субтитры · Фильтры · Бесплатно + Pro</p>
        </div>
            ...
            <div>
              ...
              <div className="bg-card border border-border rounded-xl p-3 mb-3">
                ...
                <div className="relative h-8 bg-surface rounded-lg overflow-hidden cursor-pointer"
                  ...
                  <div className="h-full bg-purple-500/30 rounded-lg"
                    style={{width:`${(currentTime/duration)*100}%`}}/>
                  {splitPoints.map((p,i)=>(
                    <div key={i} className="absolute top-0 h-full w-0.5 bg-red-400"
              ...
              {mode==='crop'&&(
                <div className="bg-card border border-border rounded-xl p-4">
                  ...
                  <p className="text-muted text-xs">Перетащи рамку на видео для точного выбора</p>
                </div>
              )}
              ...
              {mode==='split'&&(
                <div className="bg-card border border-border rounded-xl p-4">
                  ...
                  <p className="text-muted text-xs">Видео будет нарезано с сохранением звука</p>
                </div>
              )}
                ...
                <div className="bg-card border border-border rounded-xl p-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={generateSubtitles} disabled={subLoading}
                      ...
                      {subLoading?'🤖 Генерирую...':'🤖 Сгенерировать субтитры'}
                    </button>
                    {subtitles.length>0&&(
                  ...
                  {subtitles.length>0&&(
                    <div className="mt-3 space-y-1 max-h-36 overflow-y-auto">
                      {subtitles.map((s,i)=>(
                        <div key={i} className="flex gap-2 text-xs bg-surface rounded px-2 py-1.5 items-start">
            ...
            <div className="flex flex-col gap-4">
              ...
              {processing?(
                <div className="bg-card border border-border rounded-xl p-4 text-center">
                  <div className="text-purple-400 font-bold text-sm mb-2">{progress}%</div>
                  <div className="h-1.5 bg-border rounded-full overflow-hidden">
                    <div className="h-full bg-purple-500 rounded-full transition-all" style={{width:`${progress}%`}}/>
                  </div>
                  <div className="text-muted text-xs mt-2">Не закрывай страницу</div>
                </div>
              ):(
