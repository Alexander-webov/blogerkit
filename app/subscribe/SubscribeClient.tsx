        ...
        {done ? (
          <div className="animate-fadeUp">
            ...
            <p className="text-muted text-sm mb-6">
              Первый выпуск придёт в ближайший вторник. Проверь папку
              «Промоакции» если письмо не появится.
            </p>
            <div className="flex gap-3 justify-center flex-wrap">
              <Link
                href="/"
                className="px-5 py-2.5 bg-accent text-white font-bold text-sm rounded-xl hover:opacity-85 transition-opacity"
              >
                Инструменты →
              </Link>
              <Link
                href="/blog"
                className="px-5 py-2.5 border border-border text-sm rounded-xl hover:border-white/20 transition-colors"
              >
                Читать блог
              </Link>
            </div>
          ...
          <>
            {/* HERO */}
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 text-lg text-muted mb-5">
            ...
            <p className="text-muted text-sm mb-8">
              Присоединись к блогерам которые уже получают рассылку. Только
              полезное — без воды.
            </p>
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {NICHES.map((n) => (
                    <button
                      key={n}
                      onClick={() => setNiche(n)}
                      className={`px-3 py-1.5 text-lg rounded-lg border transition-all cursor-pointer ${niche === n ? "border-accent/50 bg-accent/10 text-accent" : "border-border text-muted hover:text-white bg-transparent"}`}
                    >
                      {n}
                    </button>
                  ))}
              ...
              <p className="text-muted text-lg mt-3">
                Никакого спама. Отписка в 1 клик из любого письма.
              </p>
            </div>
          </>
        )}
